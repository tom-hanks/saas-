  ...
  <div id="orderPandect">
    ...
    <!-- <div class="order_btn">
      <span
        ...
        :class="ordertypeIndex == index?'activeBtn':''"
      >{{item.type_name}}</span>
    </div>-->
            ...
            <div>
              ...
              <span>
                <i
                  ...
                  v-if="item_state_arry.page_code == 'orderPandect'&&item_state_arry.region_code=='stateTable'&&item_state_arry.public_id == item.id"
                  class="num_tip"
      ...
      <!-- <div class="active_type_tab">
        <ul>
          <li
            ...
            :key="Tindex"
            @click="isclickshow?typeTab(Tindex):''"
          >
            <div>
              {{type}}
              <span
                v-if="ordertypeIndex !=1"
              >({{Tindex == 0?orderStateNum.online_count:orderStateNum.downline_count}})</span>
          ...
          <ul>
            <li :class="{stateActive:activeTypeIndex == 0}" @click="isclickshow?typeTab(0):''">
              ...
              <i
                ...
                v-if="item_state_arry.page_code == 'orderPandect'&&item_state_arry.region_code=='stateChildTable'&&item_state_arry.code == 'stateOnline'"
                class="num_tip"
              >{{item_state_arry.num<10000?item_state_arry.num:'...'}}</i>
            </li>
            <li :class="{stateActive:activeTypeIndex == 1}" @click="isclickshow?typeTab(1):''">
              ...
              <i
                v-for="(item_state_arry,index_increaseName) in notification"
                :key="index_increaseName+'s'"
                v-if="item_state_arry.page_code == 'orderPandect'&&item_state_arry.region_code=='stateChildTable'&&item_state_arry.code == 'stateOffline'"
                class="num_tip"
    ...
    typeTab(index) {
      ...
      if (index == 0) {
        ...
        this.notification.forEach(element => {
          if (
            element.page_code == "orderPandect" &&
            element.region_code == "stateChildTable" &&
            element.code == "stateOnline"
          ) {
            this.$deleteMid_notify(element, "orderPandect");
          }
        });
      } else if (index == 1) {
      ...
      httpServer(apiSetting.ordergetAppOrder, {
        ...
        .then(response => {
          ...
          if (item.length > 0) {
            ...
            this.$getMid_notify("orderPandect");
          } else {
            this.isclickshow = true;
            if (flagName) {
              //表示搜索的时候没有数据
