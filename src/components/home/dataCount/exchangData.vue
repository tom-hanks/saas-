        ...
        <div class="order_btn" v-show="activeType">
          ...
          >
            ...
            <i
              ...
              v-if="item_state_arry.page_code == 'exchangData'&&item_state_arry.region_code=='stateTable'&&item_state_arry.public_id == item.id"
              class="num_tip"
...
export default {
  ...
  methods: {
    ...
    typeTab(index, id, type_ename) {
      ...
      //   this.getList();因为summaryExchangeReport方法里已经调用了
    },
    seachclick(a) {
      if (a == 0) {
        this.seachValue = "";
      ...
      httpServer(apiSetting.reportGetAppList, {
        ...
        .then(response => {
          ...
          if (item.length > 0) {
            ...
            this.$getMid_notify("exchangData");
            this.notification.forEach(element => {
              if (
                element.page_code == "exchangData" &&
                element.public_id == this.inquire.id
              ) {
                //   alert("删除");
                this.$deleteMid_notify(element, "exchangData");
              }
            });
          } else {
            this.isclickshow = true;
...
#exchangData .block .el-date-editor {
  width: 130px;
}
#exchangData .block .el-date-editor .el-input__inner {
  padding-left: 17px;
  padding-right: 0;
}
#exchangData .el-input__icon {
  line-height: 30px;
}
#exchangData .el-pagination__sizes .el-input .el-input__inner {
  width: 108px;
}
#exchangData .el-icon-loading {
  margin-right: -9px;
}
</style>
